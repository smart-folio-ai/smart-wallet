...
export default function Subscription() {
  ...
  return (
    <div style={{display: 'flex', flexDirection: 'column', gap: 16.8}}>
      ...
      <section id="planos" aria-label="Planos" style={{display: 'flex', flexDirection: 'column', gap: 11.2}}>
        <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 11.2, flexWrap: 'wrap'}}>
          <div>
            <h2 style={{fontFamily: 'var(--font-heading)', fontSize: 14, fontWeight: 600, margin: 0}}>Planos</h2>
            <div style={{fontSize: 11, color: 'var(--color-neutral-600)', marginTop: 2}}>Cobrança {period === 'annual' ? 'anual' : 'mensal'} · cancele quando quiser</div>
          </div>
        ...
        {plansQuery.isLoading ? (
          <div style={{...cardStyle, padding: 16.8, fontSize: 12.5, color: 'var(--color-neutral-500)'}}>Carregando planos…</div>
        ) : columns.length === 0 ? (
          <div style={{...cardStyle, padding: 16.8, fontSize: 12.5, color: 'var(--color-neutral-500)'}}>Nenhum plano disponível no momento.</div>
        ) : (
          <div className="grid grid-cols-1 items-stretch gap-[16.8px] md:grid-cols-3">
            {columns.map((column) => {
              const isCurrent = column.plan._id === currentPlanId || (!currentPlanId && column.monthlyPrice <= 0);
              ...
              return (
                <div key={column.plan._id} style={{display: 'flex'}}>
                  <PlanCard
                    ...
                    {isCurrent ? (
                      <span style={{...planCtaStyle(false), cursor: 'default', color: 'var(--color-neutral-500)'}}>Seu plano</span>
                    ) : free ? (
                      <span style={{...planCtaStyle(false), cursor: 'default', color: 'var(--color-neutral-500)'}}>Disponível pelo portal</span>
                    ) : (
                      <>
                        <button
                          type="button"
